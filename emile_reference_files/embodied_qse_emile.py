...
class EmbodiedQSEAgent(EmileAgent):
    ...
    def embodied_step(self, environment, dt=0.01):
        ...
        return {
            ...
            'context': new_context,
            'context_switched': new_context != old_context,
            'distinction_level': abs(self.symbolic.get_sigma_ema()),  # NEW top-level
            ...
            'tau_current': float(tau_current_now),     # NEW: real τc
    ...
    def _goal_to_embodied_action(self, goal, visual_field, environment=None):
        ...
        if np.random.random() < 0.7 and np.mean(center_vision) > 0.6:
            return 'examine', 1.0
        # --- END CURIOSITY FIX ---
